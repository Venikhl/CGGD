...
void cg::renderer::rasterization_renderer::init()
{
	...
	render_target = std::make_shared<cg::resource<float>>(settings-> width, settings->height);
...
void cg::renderer::rasterization_renderer::render()
{
	...
	rasterizer->pixel_shader = [](cg::vertex data, float z){
		return cg::color(data.ambient_r, data.ambient_b, data.ambient_g);
	};
	...
	for (size_t shape_id = 0; shape_id < model->get_index_buffers().size(); shape_id++)
	{
		...
		rasterizer->draw(model->get_vertex_buffers()[shape_id]->count(), 0);
	}
