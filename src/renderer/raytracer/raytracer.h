...
namespace cg::renderer
{
	...
	inline void raytracer<VB, RT>::build_acceleration_structure()
	{
		...
		for (size_t shape_id=0; shape_id < index_buffers.size(); shape_id++)
		{
			...
			{
				triangle<VB> triangle (vertex_buffer->item(index_buffer->item(index_id++)), vertex_buffer->item(index_buffer->item(index_id++)), vertex_buffer->item(index_buffer->item(index_id++)));
